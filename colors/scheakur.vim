...
function! s:set_dark_colors()
	...
	let s:base_args = ['#f0f0e5', '#2e2e2e', 'none', 255, 234, 'none']
	...
	call s:hi('NonText', '#7878ba', _, s:base)
	call s:hi('Question', '#008080')
	call s:hi('IncSearch', x, '#f4b3c2', s:base, s:base, 218)
	call s:hi('Search', x, '#e9e7ac')
	...
	call s:hi('TabLineSel', s:frame, _, s:base)
	call s:hi('Title', '@Special', _, s:base)
	call s:hi('Pmenu', s:base, '#124055', _, _, 231)
	call s:copy('PmenuSel', '@IncSearch')
	call s:hi('PmenuSbar', _, '@Pmenu')
	call s:hi('PmenuThumb', '@PmenuSel')
	call s:hi('Comment', '#506a78', _)
	...
	call s:hi('Constant', '#9a79b6')
	call s:hi('Identifier', '#70a395')
	call s:hi('Statement', '#91b5d4', _, s:base)
	call s:hi('String', '#b7b063')
	call s:hi('PreProc', '#5f94ca')
	call s:hi('Operator', '@PreProc')
	call s:hi('Special', '#779411')
	call s:hi('Type', '#d38d6b', _, s:base)
	call s:hi('Function', '#e279c8')
	call s:hi('MatchParen', '#0e8ed3', '#dbf2ff')
	call s:hi('Ignore', '#666666')
	call s:hi('Todo', '#fdfec9', '#4d4214', _, _, 229)
	call s:hi('Error', '#ffe3e5', '#b24a25', _, _, 223)
	call s:hi('Underlined', '#7393e6')
	call s:hi('Tag', '#a25a09')
	call s:copy('WildMenu', '@IncSearch')
	call s:hi('SignColumn', s:base, _)
	call s:hi('SpellBad', '@Error', '@Error', 'undercurl', _, '@Error')
