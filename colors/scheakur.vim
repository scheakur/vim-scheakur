...
function! s:set_highlight()
	...
	call s:hi('Cursor', x, '#f39812')
	call s:hi('CursorIM', '#fededa', '#47885e')
	call s:hi('CursorLine', x, '#fafaf7')
	call s:hi('CursorColumn', x, '@CursorLine')
	call s:hi('Directory', '#1177dd')
	call s:hi('Folded', '#04530d', '#d0ead0')
	call s:hi('FoldColumn', '@Folded', '@Folded')
	call s:hi('LineNr', '#567686', '#e2e2d0', x, 236)
	...
	call s:hi('NonText', '#7878ba', x, s:base)
	...
	call s:hi('Visual', x, '#cce0ef', x, x, 153)
	call s:hi('TabLine', '@StatusLine', '@StatusLine', '@StatusLine')
	call s:hi('TabLineFill', '@StatusLine', '@StatusLine', '@StatusLine')
	call s:hi('TabLineSel', s:frame, x, s:base)
	call s:hi('Title', '@Special', x, s:base)
	call s:hi('Pmenu', s:base, '#f6e4e7', x, x, 231)
	call s:copy('PmenuSel', '@IncSearch')
	call s:hi('PmenuSbar', x, '@Pmenu')
	call s:hi('PmenuThumb', '@PmenuSel')
	call s:hi('Comment', '#1e7b88', x)
	call s:hi('ColorColumn', x, '#dfd6d1')

	call s:hi('Constant', '#a25a09')
	call s:hi('Number', '#6b118a')
	call s:hi('Identifier', '#1a683a')
	call s:hi('Statement', '#0c71d5', x, s:base)
	call s:hi('String', '#0b3fad')
	call s:hi('PreProc', '@Number')
	call s:hi('Conditional', '@Function')
	call s:hi('Special', '#141784')
	call s:hi('Type', '#bd3b09', x, s:base)
	call s:hi('Repeat', '@Identifier')
	call s:hi('Function', '#cb1265')
	call s:hi('MatchParen', '#0e8ed3', '#dbf2ff')
	call s:hi('Ignore', '#666666')
	call s:hi('Todo', '#4d4214', '#fdfec9', x, x, 229)
	call s:hi('Error', '#d1160b', '#ffe3e5', x, x, 223)
	call s:hi('Underlined', '#2358ba')
	call s:copy('WildMenu', '@IncSearch')
	call s:hi('SignColumn', s:base, x)
	call s:hi('x', '@Error', '@Error', 'undercurl', x, '@Error')
	call s:hi('SpellCap', '@String', '@MatchParen', 'undercurl')
	call s:hi('SpellRare', '@Folded', '@Folded', 'undercurl')
	call s:hi('SpellLocal', '@Todo', '@Todo', 'undercurl', x, '@Todo')
	call s:hi('DiffAdd', s:base, '@SpellRare')
	call s:hi('DiffChange', s:base, '@SpellLocal', x, x, '@Todo')
	call s:hi('DiffDelete', s:base, '@x', x, x, '@Error')
